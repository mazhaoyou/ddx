<template>
	<!-- NumberBox 数字输入框组件
	* numBoxWidth 为底部数字框样式选择开关，
	* numBoxWidth为0时，未开始竞拍页面数字框
	*  -->
	<view class="uni-numbox">
		<view class="uni-numbox__minus" :class="{'uni-numbox--disabled': disableSubtract||disabled}" @click="_calcValue('subtract')">-</view>
		<input v-if="numBoxWidth==0" class="uni-numbox__value num-box-width" type="number" :disabled="disabled" :value="inputValue" @blur="_onBlur">
		<input v-else class="uni-numbox__value" type="number" :disabled="disabled" :value="inputValue" @blur="_onBlur">
